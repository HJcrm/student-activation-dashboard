...
BASE_URL = "https://operation.hakzzongpro.com/institution-activation-logs/"
FILENAME_PREFIX = "학원활성화현황_"
...
with concurrent.futures.ThreadPoolExecutor(max_workers=10) as ex:
    futs = {ex.submit(download_file, d): d for d in dates}
    for i, f in enumerate(concurrent.futures.as_completed(futs), 1):
        ...
        if i % 50 == 0 or i == len(dates):
            print(f"  {i}/{len(dates)} | OK:{ok} Skip:{skip} Fail:{fail}")
